...
final class BVCTests: XCTestCase {

    func testIf() throws {
        let nes = Nes()
        nes.cpu.registers.overflowFlag = false
        let registers = nes.cpu.registers
        nes.memory.storage[0x00] = nes.cpu.opcode(.bvc, .relative)
    ...
    func testIfNewPage() throws {
        ...
        nes.cpu.registers.overflowFlag = false
        nes.memory.storage[0x00FC] = nes.cpu.opcode(.bvc, .relative)
    ...
    func testIfBackwards() throws {
        ...
        nes.cpu.registers.pc = 0x00FC
        nes.cpu.registers.overflowFlag = false
        nes.memory.storage[0xFC] = nes.cpu.opcode(.bvc, .relative)
    ...
    func testElseNewPage() throws {
        ...
        nes.cpu.registers.overflowFlag = true
        nes.memory.storage[0x00FE] = nes.cpu.opcode(.bvc, .relative)

...
final class BPLTests: XCTestCase {

    func testIf() throws {
        let nes = Nes()
        nes.cpu.registers.signFlag = false
        let registers = nes.cpu.registers
        nes.memory.storage[0x00] = nes.cpu.opcode(.bpl, .relative)
    ...
    func testIfNewPage() throws {
        ...
        nes.cpu.registers.signFlag = false
        nes.memory.storage[0x00FC] = nes.cpu.opcode(.bpl, .relative)
    ...
    func testIfBackwards() throws {
        ...
        nes.cpu.registers.pc = 0x00FC
        nes.cpu.registers.signFlag = false
        nes.memory.storage[0xFC] = nes.cpu.opcode(.bpl, .relative)
    ...
    func testElseNewPage() throws {
        ...
        nes.cpu.registers.signFlag = true
        nes.memory.storage[0x00FE] = nes.cpu.opcode(.bpl, .relative)

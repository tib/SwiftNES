...
final class INYTests: XCTestCase {
    ...
    func testImplicit() throws {
        ...
        nes.memory.storage[0x00] = nes.cpu.opcode(.iny, .implicit)
        nes.start(cycles: 2)
        XCTAssertEqual(nes.cpu.registers.y, 0x1)
    ...
    func testImplicitNegative() throws {
        let nes = Nes()
        let registers = nes.cpu.registers
        nes.cpu.registers.y = 0b10001000
        nes.cpu.registers.zeroFlag = true
        nes.cpu.registers.signFlag = false
        nes.memory.storage[0x00] = nes.cpu.opcode(.iny, .implicit)
        nes.start(cycles: 2)
        XCTAssertEqual(nes.cpu.registers.y, 0b10001001)
        XCTAssertFalse(nes.cpu.registers.zeroFlag)

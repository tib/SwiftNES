...
final class STXTests: XCTestCase {
    ...
    func testZeroPage() throws {
        ...
        nes.memory.storage[0x00] = nes.cpu.opcode(.stx, .zeroPage)
    ...
    func testZeroPageY() throws {
        ...
        nes.memory.storage[0x00] = nes.cpu.opcode(.stx, .zeroPageY)
        nes.memory.storage[0x01] = 0x42
        nes.start(cycles: 4)
    ...
    func testAbsolute() throws {
        ...
        let registers = nes.cpu.registers
        nes.cpu.registers.x = 0x2F
        nes.memory.storage[0x00] = nes.cpu.opcode(.stx, .absolute)
        nes.memory.storage[0x01] = 0x80

...
final class ROLTests: XCTestCase {
    ...
    func testAccumulatorZeroWithCarry() throws {
        ...
        nes.memory.storage[0x00] = nes.cpu.opcode(.rol, .accumulator)
        let cycles = 2
    ...
    func testAccumulatorNegative() throws {
        let nes = Nes()
        nes.cpu.registers.carryFlag = false
        nes.cpu.registers.a = 0b01000000
        nes.memory.storage[0x00] = nes.cpu.opcode(.rol, .accumulator)
        let cycles = 2
        nes.start(cycles: cycles)
    ...
    func testZeroPage() throws {
        ...
        nes.memory.storage[0x00] = nes.cpu.opcode(.rol, .zeroPage)
    ...
    func testZeroPageX() throws {
        ...
        nes.memory.storage[0x00] = nes.cpu.opcode(.rol, .zeroPageX)
        nes.memory.storage[0x01] = 0x42
    ...
    func testAbsolute() throws {
        let nes = Nes()
        nes.cpu.registers.a = 1
        nes.memory.storage[0x00] = nes.cpu.opcode(.rol, .absolute)
    ...
    func testAbsoluteX() throws {
        ...
        nes.memory.storage[0x00] = nes.cpu.opcode(.rol, .absoluteX)
        nes.memory.storage[0x01] = 0x80
        nes.memory.storage[0x02] = 0x01

...
final class CPXTests: XCTestCase {
    ...
    func testImmediate() throws {
        ...
        nes.cpu.registers.x = 26
        nes.memory.storage[0x00] = nes.cpu.opcode(.cpx, .immediate)
        nes.memory.storage[0x01] = 26
        let cycles = 2
        nes.start(cycles: cycles)
        XCTAssertEqual(nes.cpu.registers.x, 26)
    ...
    func testZeroPage() throws {
        ...
        nes.cpu.registers.signFlag = true
        nes.cpu.registers.x = 48
        nes.memory.storage[0x00] = nes.cpu.opcode(.cpx, .zeroPage)
        nes.memory.storage[0x01] = 0x42
        nes.memory.storage[0x42] = 26
        let cycles = 3
        nes.start(cycles: cycles)
        XCTAssertEqual(nes.cpu.registers.x, 48)
        XCTAssertTrue(nes.cpu.registers.carryFlag)
    ...
    func testAbsolute() throws {
        ...
        nes.cpu.registers.x = 8
        nes.memory.storage[0x00] = nes.cpu.opcode(.cpx, .absolute)
        nes.memory.storage[0x01] = 0x80
        nes.memory.storage[0x02] = 0x01
        nes.memory.storage[0x0180] = 26
        let cycles = 4
        nes.start(cycles: cycles)
        XCTAssertEqual(nes.cpu.registers.x, 8)
